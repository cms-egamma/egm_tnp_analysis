...
def getAllEffi( info, bindef ):
    ...
    if not info['mcNominal'] is None and os.path.isfile(info['mcNominal']):
        ...
        with open("printout_mcNominal.txt", "w") as file:
            file.write("Passing histogram:\n")
            for i in range(bin1, bin2 + 1):
                file.write("Bin {}: Content = {}, Error = {}\n".format(i, hP.GetBinContent(i), hP.GetBinError(i)))

            file.write("Failing histogram:\n")
            for i in range(bin1, bin2 + 1):
                file.write("Bin {}: Content = {}, Error = {}\n".format(i, hF.GetBinContent(i), hF.GetBinError(i)))

        if (bindef['cut'] == "tag_Ele_pt > 32 && abs(tag_sc_eta) < 2.5 && el_q*tag_Ele_q < 0 && passingHEEPV70==1 && abs(el_eta) < 2.5 && el_pt > 5 && el_sc_eta >= 1.566000 && el_sc_eta < 2.500000 && el_pt >= 350.000000 && el_pt < 500.000000"):
            with open("printout_mcNominal.txt", "a") as file:
